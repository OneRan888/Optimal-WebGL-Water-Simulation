...
function Water() {
  ...
  this.updateShader = new GL.Shader(vertexShader, '\
    uniform sampler2D texture;\
    uniform vec2 delta;\
    uniform float fluidity; /* 0 = very viscous, 1 = normal water */\
    varying vec2 coord;\
    void main() {\
      /* get vertex info */\
      vec4 info = texture2D(texture, coord);\
      ...
      /* fluidity: 1 = 水, 0 = 稠液体 */\
      /* 物理直觉：波速略受影响，阻尼差异很大 */\
      float speed   = mix(0.6, 2.0, fluidity);   /* 波传播速度 */\
      float damping = mix(0.80, 0.995, fluidity);/* 阻尼：粘度越高，能量越快被吃掉 */\
      \
  ...
  this.calmShader = new GL.Shader(vertexShader, '\
    uniform sampler2D texture; /* 保留 uniform 以兼容接口，实际上不用 */\
    varying vec2 coord;\
    void main() {\
...
Water.prototype.setFluidity = function(value) {
...
Water.prototype.calm = function() {
  var this_ = this;
  this.textureB.drawTo(function() {
    this_.textureA.bind();
    this_.calmShader.draw(this_.plane);
  });
  this.textureB.swapWith(this.textureA);
};
